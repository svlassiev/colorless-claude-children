...
Surprises / deviations: (1) `development` gcloud config did not match CLAUDE.md's description — CLAUDE.md may need a correction after the new `personal` config is in place. (2) Naive `gh` calls would silently operate as `a work username` due to the env-var-set token — non-obvious. (3) IDE linter persistently flags the cost-gates table layout (valid GFM, info-level — ignored).
