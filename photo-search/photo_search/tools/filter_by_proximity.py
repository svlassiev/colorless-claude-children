...
def execute(args: Args, metas: list[dict]) -> ProximityFilter | None:
    ...
    # Anchors: coordinates of photos labeled with one of the canonicals.
    ...
    for m in metas:
        ...
        if any(_norm(pn) in canonicals_n for pn in names):
            anchors.append(coord)

    if not anchors:
        # Recognized name, but none of its photos carry a coordinate — we
        # have no point to measure from. Treat as unlocatable.
        return None
